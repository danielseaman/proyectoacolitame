...
public interface EmpresaRepositorio extends JpaRepository<Empresa,Integer> {
    @Query(value="select * from empresa  where empresa.nombre like '%?1%'",nativeQuery=true)//Probar si vale con'' numero4
    ...
    @Query(value="select id_empresa,nombre,correo from empresa  where empresa.id_categoria=?1",nativeQuery=true)//numero 2
    List<Empresa> findByCategoria(int categoria);
}
