...
public interface ProductoRepositorio extends JpaRepository<Producto,Integer> {
    ...
    @Query(value="select producto.id_producto,producto.nombre,producto.precio,producto.descripcion,producto.id_empresa from categoria,empresa,producto  where empresa.id_categoria=categoria.id_categoria and categoria.id_categoria=?1 and empresa.id_empresa=producto.id_empresa",nativeQuery=true)//numero3
    List<Producto> findByCategoria(Integer idCategoria);
    @Query(value="select * from producto  where producto.nombre like '%?1'",nativeQuery=true)//numero5
