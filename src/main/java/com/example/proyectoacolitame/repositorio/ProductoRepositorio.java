...
public interface ProductoRepositorio extends JpaRepository<Producto,Integer> {
    ...
    @Query(value="select producto.id_producto,producto.nombre,producto.bytes_foto,producto.precio,producto.descripcion,empresa.id_empresa,empresa.nombre as nombre2,empresa.bytes_foto as foto2 from producto FULL OUTER JOIN empresa USING (id_empresa) where lower(producto.nombre) like CONCAT('%',lower(?1),'%')",nativeQuery=true)//numero5
